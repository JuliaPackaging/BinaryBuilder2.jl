...
@testset "MultiJLLOutput" begin
    ...
    zlib_slim_path = artifact_path(only(package_result.config.named_extractions["ZlibSlim"]))
    zlib_path = artifact_path(only(package_result.config.named_extractions["Zlib"]))

    @test ispath(joinpath(zlib_slim_path, "lib", "libz.so.1"))
    @test ispath(joinpath(zlib_path, "lib", "libz.so.1"))

    @test !isfile(joinpath(zlib_slim_path, "include", "zlib.h"))
    ...
    @test "ZlibSlim_jll" ∈ get_deps(package_result)

    package_result = get_package_result(meta, "ZlibSlim")
    @test package_result.status == :success
