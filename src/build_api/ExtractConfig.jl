...
function extract_spec_hash(build_hash::SHA1Hash, extract_script::String, products::Vector{<:AbstractProduct})
    ...
    for product in sort(products; by = p->p.varname)
        ...
        # A library's static archive is subordinate, and so would otherwise be
...
function BinaryBuilderAuditor.audit!(config::ExtractConfig, artifact_dir::String; verbose::Bool = AbstractBuildMeta(config).verbose, kwargs...)
    ...
    @timeit config.to "audit" begin
        ...
        # A dependency's libraries, however they are linked: an archive is as much a
        # library a dependent can link against as a shared object is.
        get_library_products(jart::JLLBuildInfo) = AbstractJLLProduct[
            x for x in jart.products if isa(x, JLLLibraryProduct) || isa(x, JLLStaticLibraryProduct)]
