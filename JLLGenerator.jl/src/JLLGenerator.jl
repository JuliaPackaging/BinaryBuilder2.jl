...
essentially a fusion of the `BuildConfig` and `PackageConfig` structures in `BB2`, but
is distinct in order to maintain separation between the two packages, and to make it
easier for non-BB2 users to make use of this package if needed.
"""
...
function generate_jll(out_dir::String, info::JLLInfo; clear::Bool = true, build_metadata::Dict{String,String} = Dict{String,String}())
    ...
    open(joinpath(out_dir, "README.md"); write=true) do io
        ...
        This is an autogenerated package constructed using [JLLGenerator.jl](https://github.com/JuliaPackaging/BB2.jl/tree/main/JLLGenerator.jl).
        """)
